...
resource "aws_s3_bucket" "loki-chunks" {
  ...
  tags = merge(
    {
      Name = format("%s-%s-loki-chunks", var.project_name, data.aws_caller_identity.current.account_id)
    },
    var.common_tags
  )
}
...
resource "aws_s3_bucket" "loki-admin" {
  ...
  tags = merge(
    {
      Name = format("%s-%s-loki-admin", var.project_name, data.aws_caller_identity.current.account_id)
    },
    var.common_tags
  )
}
...
resource "aws_s3_bucket" "loki-ruler" {
  ...
  tags = merge(
    {
      Name = format("%s-%s-loki-ruler", var.project_name, data.aws_caller_identity.current.account_id)
    },
    var.common_tags
  )
}
