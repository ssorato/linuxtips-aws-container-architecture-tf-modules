...
resource "aws_eks_pod_identity_association" "argo_server" {
  ...
  tags = merge(
    {
      Name = format("%s-argocd-server", var.project_name)
    },
    var.common_tags
  )
}
...
resource "aws_eks_pod_identity_association" "argo_application_controller" {
  ...
  tags = merge(
    {
      Name = format("%s-argocd-application-controller", var.project_name)
    },
    var.common_tags
  )
}
...
resource "aws_eks_pod_identity_association" "argo_applicationset_controller" {
  ...
  tags = merge(
    {
      Name = format("%s-argocd-applicationset-controller", var.project_name)
    },
    var.common_tags
  )
}