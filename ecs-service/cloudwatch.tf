...
resource "aws_cloudwatch_metric_alarm" "cpu_high" {
  ...
  alarm_name = format("ecs-%s-%s-cpu-scale-out", var.ecs_name, var.ecs_service_name)
  ...
  dimensions = {
    ClusterName = format("ecs-%s", var.ecs_name)
  ...
  tags = merge(
    {
      Name = format("ecs-%s-%s-cpu-scale-out", var.ecs_name, var.ecs_service_name)
    },
    var.common_tags
  )
}
...
resource "aws_cloudwatch_metric_alarm" "cpu_low" {
  ...
  alarm_name = format("ecs-%s-%s-cpu-scale-in", var.ecs_name, var.ecs_service_name)
  ...
  tags = merge(
    {
      Name = format("ecs-%s-%s-cpu-scale-in", var.ecs_name, var.ecs_service_name)
    },
    var.common_tags
  )
}
